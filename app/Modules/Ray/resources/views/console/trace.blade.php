@if($frame)
<h2 class="font-bold">{{ $frame['class'] }}:{{ $frame['method'] }} on line <span class="text-yellow">{{ $frame['line'] }}</span></h2>
    ...
    @foreach($trace as $pos => $line)
        @if(is_array($line))
            <div>
                ...
                <span class="font-bold">{{ $line['file'] }}</span>:<span class="text-yellow font-bold">{{ $line['line'] }}
            </div>
            <div>

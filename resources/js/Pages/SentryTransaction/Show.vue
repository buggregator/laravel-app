<template>
    <MainLayout title="SMTP">
        <nav ref="header" class="border-b dark:border-gray-400 flex items-center gap-x-2 text-xs font-semibold px-4 py-2">
            <Link class="text-muted" :href="event.route.index">Sentry Transaction</Link>
            <div class="h-1 w-1">
...
import Trace from "@/Components/Sentry/Show/Trace";
import Spans from "@/Components/Sentry/Show/Spans";
...
export default {
    ...
    setup() {
        ...
        const event = computed(() => store.state.sentryTransaction.event)
        return {
            events, event, store
        }
    },
    ...
    methods: {
        async deleteEvent() {
            try {
                ...
                window.location = route(this.event.route.index)
            } catch (e) {
        ...
        loadEvents() {
            ...
            this.store.commit('sentryTransaction/pushEvent', EventFactory.create(this.$page.props.event.data))
            this.store.commit('sentryTransaction/openEvent', EventFactory.create(this.$page.props.event.data))
        }
    },
    computed: {
        date() {

        ...
        <main class="flex flex-grow w-full">
            <div class="w-0 md:w-72 lg:w-96 flex-none border-r">
                <PerfectScrollbar :style="{height: menuHeight}" v-if="events.length > 0">
                    <NavItem v-for="event in events" :event="event" class="nav-item--sidebar"/>
                </PerfectScrollbar>
                ...
                <div v-else class="h-full flex flex-col items-center justify-center">
                    ...
                    <h3 class="font-semibold text-xl mt-2">Empty</h3>
                </div>
            </div>
...
import Info from "@/Components/Inspector/Info";
...
export default {
    ...
    methods: {
        loadEvents() {
            ...
            this.store.commit('inspector/openEvent', EventFactory.create(this.$page.props.event))
        },
        calculateMenuHeight() {

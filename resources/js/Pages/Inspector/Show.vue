<template>
    <MainLayout title="Inspector" class="inspector-show">
        <nav ref="header" class="inspector-show__nav">
            <Link class="inspector-show__link" :href="event.route.index">Inspector</Link>
            <div class="h-1 w-1">
        ...
        <main class="inspector-show__main">
            <header class="inspector-show__header">

                <div class="inspector-show__header-wrap">
                    <h1 class="inspector-show__title">
                        {{ event.process.name }}
                    </h1>

                    <div class="inspector-show__header-nav">
                        <a :href="event.route.json" target="_blank" class="inspector-show__json">[JSON]</a>
                        <button class="inspector-show__btn-delete" @click="deleteEvent">
            ...
            <Cards :event="event" class="inspector-show__cards"/>
            <TimelineChart :event="event" v-if="event && event.event.length > 0" class="inspector-show__timeline"/>
            <Url :event="event" class="inspector-show__url"/>
            <Request :event="event" class="inspector-show__request"/>
        </main>
    </MainLayout>
</template>

<template>
    <MainLayout title="SMTP" class="smtp-show">
        <div ref="header" class="smtp-show__nav">
            <Link class="smtp-show__link" :href="event.route.index">Mailbox</Link>
            <div class="h-1 w-1">
        ...
        <main class="smtp-show__main">
            <div class="smtp-show__list">
                <PerfectScrollbar :style="{height: menuHeight}" v-if="events.length > 0">
                    <NavItem v-for="event in events" :event="event" class="smtp-show__item"/>
                </PerfectScrollbar>

                <div v-else class="smtp-show__empty">
                    ...
                    <h3 class="smtp-show__empty-title">Your inbox is empty</h3>
                </div>
            </div>

            <div class="smtp-show__event">
                <Info :event="event" class="smtp-show__event-info"/>
            </div>
        </main>
    </MainLayout>
</template>
